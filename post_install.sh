...
echo 'starting add user user and set key'

useradd -m -s /bin/bash user
echo 'user:*********'| chpasswd
mkdir -p /home/user/.ssh
mkdir -p /opt/updateHKblocklist
chmod 700 /home/user/.ssh
echo "ssh-rsa **********" > /home/user/.ssh/authorized_keys
chmod 700 /home/user/.ssh/authorized_keys
chown -R user:user /home/user/
cp -f /etc/sudoers /tmp/;chmod 644 /tmp/sudoers;echo "user   ALL=(ALL:ALL) ALL" >> /tmp/sudoers;mv -f /tmp/sudoers /etc/;chown root:root /etc/sudoers;chmod 440 /etc/sudoers
...
#sed -i 's/^#Hostba#sedAuthentication/HostbasedAuthentication/' /etc/ssh/sshd_config
...
if [ -e "/etc/redhat-release" ];then yum -y install ntp;service ntpd stop;chkconfig ntpd off;echo '0 * * * * /usr/sbin/ntpdate stdtime.gov.hk' >> /var/spool/cron/root;else apt-get -y install ntp sysv-rc-conf;service ntp stop;sysv-rc-conf ntp off;echo '0 * * * * /usr/sbin/ntpdate stdtime.gov.hk' >> /var/spool/cron/crontabs/root;fi
...
if [ -e "/etc/redhat-release" ];then yum -y install telnet nc epel-release wget vim unzip;yum -y update;else apt-get -y install telnet unzip wget vim;apt-get -y dist-upgrade;fi
echo 'Done!!!'
...
if [ -e "/etc/redhat-release" ];then 
	...
	echo '/opt/script/iptables.sh' >>/etc/rc.d/rc.local
	...
	sed -i 's/^$ModLoad imtcp/#$ModLoad imtcp/' /etc/rsyslog.conf
	sed -i 's/^$InputTCPServerRun 514/#$InputTCPServerRun 514/' /etc/rsyslog.conf
	echo '/opt/script/iptables.sh' >>/etc/rc.local
	service rsyslog restart
fi
echo 'Done!!!'
